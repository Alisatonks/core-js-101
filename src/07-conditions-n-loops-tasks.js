...
function getSumBetweenNumbers(n1, n2) {
  const array = Array(n2 - n1 + 1).fill(n1).map((el, index) => el + index);
...
function doRectanglesOverlap(rect1, rect2) {
  const x = (rect1.width + rect1.left) > rect2.left;
  const y = (rect1.height + rect1.top) > rect2.top;
...
function getMatrixProduct(m1, m2) {
  ...
  const array = new Array(len1).fill(null);
  for (let i = 0; i < array.length; i += 1) {
    array[i] = new Array(m2[i].length).fill(null);
    for (let j = 0; j < len1; j += 1) {
      let sum = 0;
      for (let y = 0; y < len2; y += 1) {
        sum += m1[i][y] * m2[y][j];
        array[i][j] = sum;
      }
    }
